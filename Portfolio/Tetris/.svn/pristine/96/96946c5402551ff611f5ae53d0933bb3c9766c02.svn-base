...
public class Board extends Observable {
    ...
    /*
     * Fields added by Riley Gratzer for variant modes and holding.
     */
    /**
     * Boolean flag for treadmill mode.
     */
    private boolean myTreadmill;

    /**
     * Hold Piece.
     */    
    private MovableTetrisPiece myHold;
    ...
    public Board(final int theWidth, final int theHeight) {
        ...
        /*
         * Initialize added treadmill flag.
         */
        myTreadmill = false;
    }
    ...
    public void down() {
        if (!move(myCurrentPiece.down())) {
            ...
            /*
             * Treadmill variant applied here.
             */
            if (myTreadmill) {
                for (final Color[] ca: myFrozenBlocks) {
                    final Color first = ca[0];
                    System.arraycopy(ca, 1, ca, 0, getWidth() - 1);
                    ca[getWidth() - 1] = first;
                }
            }
    ...
    /*
     * Added methods to allow setting of variant mode flags. 
     */
    /**
     * This method allows toggling of Treadmill flag.
     */
    ...
    /**
     * Returns the boolean value of the Treadmill setting.
     * 
     * @return the treadmill setting.
     */
    ...
    /*
     * Hold method added for piece hold feature.
     */
    /**
     * Allows for a piece to be stored and called back later.
     */
    public void hold() {
        if (myHold == null) {
            myHold = new MovableTetrisPiece(myCurrentPiece.getTetrisPiece(), 
                                  myCurrentPiece.getPosition(), myCurrentPiece.getRotation());
            myCurrentPiece = getNextMovablePiece(false);
        } else {
            int startY = myHeight - 1;
            if (myHold.getTetrisPiece() == TetrisPiece.I) {
                startY--; 
            }
            final MovableTetrisPiece 
                                 temp = new MovableTetrisPiece(myCurrentPiece.getTetrisPiece(),
                                 new Point((myWidth - myCurrentPiece.getWidth()) / 2, startY)); 
            
            myCurrentPiece = new MovableTetrisPiece(myHold.getTetrisPiece(),
                        new Point((myWidth - myHold.getWidth()) / 2, startY));
            
            myHold = new MovableTetrisPiece(temp.getTetrisPiece(),
                                          new Point((myWidth - temp.getWidth()) / 2, startY));
        }
        setChanged();
        notifyObservers(myHold);
    }
